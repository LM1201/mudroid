...
import MuDroid.Singleton.InheritanceRelation;
import mujava.util.DirFileFilter;
import mujava.util.ExtensionFilter;
import mujava.util.InheritanceINFO;

public class RecordInheritanceRelationToXml extends RecordInheritanceRelation {
	int level=13;
	...
	public  static void  main(String[] args) {
		...
		run(new RecordInheritanceRelationToXml(),args);
			...
			for(File fileIndex:files)
			{
				...
				else
				{
					...
					if(fileIndex.isDirectory())
					{
						...
						writer.print("<Dir"+" path=\""+fileIndex.getAbsolutePath()+"\""+" name=\""+fileIndex.getName()+"\" "+"depth=\""+count+"\">"); 	    
					...
					writer.println("<File"+" path=\""+fileIndex.getAbsolutePath()+"\""+" name=\""+fileIndex.getName()+"\" "+"depth=\""+count+"\">"+"</File>");
					lastcount=count;
					}
				}
			}
	 ...
	 public  String[] getAllClassNames(String[] result,String dir)
	   {
	      ...
	      if (classF != null)
	      {
	         ...
	         for(int k=0; k<classF.length; k++) 
	         {
	            ...
	            classes[k] = temp.substring(this.class_Debug.length()+1, temp.length()-".class".length());
...
public  void recordInheritanceRelation() throws Exception
{
	...
	list=InheritanceRelation.getInstance().getJarsList();
...
//	addURL("F:/muandroid3/AndroidApp-master/app/build/intermediates/exploded-aar/com.android.support");
	if (classes == null)
	{
		...
		System.err.println(" Please check the directory  " + this.class_Debug + " and be sure that MuJava_HOME is set correctly (without a trailing slash) in mujava.config.");
...
public  String[] getAllClassNamesInJARSlist(String[] result,List<String> list) throws IOException
{
   String[] classes;//为何又用string[]去存？
   String temp;
   String [] classF=null;
   ...
   if (classF != null)
   {
      ...
      //  System.out.println(""+temp2.indexOf(".class"));
        if(temp2.indexOf(".class")>0)
        {
         ...
         classes[length] = temp2.substring(0, temp2.indexOf(".class"));
