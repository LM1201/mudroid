...
//对整个基础表进行遍历，如果是就返回这个结点。找不到就返回为空
        for (int i=0; i<classInfo.length; i++)
        {
        	...
        	// ;
           if (classInfo[i].getClassName().substring(0,classInfo[i].getClassName().lastIndexOf('.')).equals(package_name)) 
           {
        ...
        for (int i=0; i<classInfo.length; i++)
        {
           ...
           if (classInfo[i].getClassName().substring(0,classInfo[i].getClassName().lastIndexOf('.')).equals(package_name)) 
           {
        	   if (!classInfo[i].getClassName().equals(class_name)) 
        	   {
        		   if(classInfo[i].getParentName().equals(InheritanceINFOthis.getParentName()))
