...
public class MutantJPanel extends JPanel 
				implements ActionListener
{
	...
	JLabel[] jlArray=new JLabel[]
			{
			new JLabel("<html><font size=3>[1] Select files to test</font></html>"),
			new JLabel("<html><font size=3>[2] Select mutation operators to apply</font></html>"),
			new JLabel("<html><font size=3>[3] Push \"generate mutants\" button</font></html>"),
			new JLabel("<html><font size=3>[4] Wait with endurance. ^^;</font></html>"),
	 		};
	String filestring="F:/muandroid3/AndroidApp-master";

	String projectName="";
	MutantJPanel(String projectName)
	{
	...
	void init()
	{
		...
		xmlFilePanel.setBorder(new TitledBorder("All Xml Files which can be mutanted"));
		//filePanel.add(xmlFilePanel);
		xmlFilePanel.setLayout(new BoxLayout(xmlFilePanel,BoxLayout.PAGE_AXIS));
		FileTableModel fTableModel0 = new FileTableModel(getNewTragetFiles("manifest"),"manifest");
	    ...
	    FileTableModel fTableModel2 = new FileTableModel(getNewTragetFiles("xml"),"xml");
		...
		javaFilePanel.setBorder(new TitledBorder("All Java Files which can be mutanted"));
		//filePanel.add(javaFilePanel);
		javaTbedPane.addTab("Java Files", javaFilePanel);
		javaFilePanel.setLayout(new BoxLayout(javaFilePanel,BoxLayout.PAGE_AXIS));
		
		FileTableModel fTableModel = new FileTableModel(getNewTragetFiles("java"),"java");
	  ...
	  //  opTbedPane.setTabLayoutPolicy(JTabbedPane.VERTICAL);
	   ...
	   // OpPanel.add(amPanel);
		...
		usagePanel.setBorder(new TitledBorder("The thing needed todo in this Panel"));
		...
		anPanel.setBorder(new TitledBorder("Select ALL or None of all files and Mutant Op"));
		
		anPanel.add(allBt);
		anPanel.add(noneBt);
		getMutant.setBackground(Color.CYAN);
		 getMutantPanel.add(getMutant);
		 getMutantPanel.setBorder(new TitledBorder("Generate mutants of choosed files in the choosed Op"));
		...
		String[] args=new String[]{"F:/muandroid3/AndroidApp-master"};
		try {
			ReadFile.main(null);
	...
	public static void main(String[] args)
	{
		...
		MutantJPanel fjp=new MutantJPanel();
		jf.add(fjp);
	...
	public void actionPerformed(ActionEvent e) {
		...
		{ 
			...
			FileTableModel table = (FileTableModel)xmlFileTable.getModel();
		      ...
		      FileTableModel table2 = (FileTableModel)manifestFileTable.getModel();
		...
		if(e.getSource()==this.getMutant)
		{
			...
			FileTableModel fTableModel = (FileTableModel)javaFileTable.getModel();
  			String[] javafile_list = fTableModel.getSelectedFiles();
  			FileTableModel fTableModel2 = (FileTableModel)xmlFileTable.getModel();
  			String[] xmlfile_list = fTableModel2.getSelectedFiles();
  			
  			FileTableModel fTableModel3 = (FileTableModel)manifestFileTable.getModel();
...
//			}
  			...
  			pjf.setSome(javafile_list, xmlfile_list, manifestfile_list, args, java_SRC_PATH, xml_SRC_PATH, manifest_SRC_PATH);;
  			pjf.setVisible(true);
  			pjf.stop();
			pjf.strat();
		}
	}
	static String java_SRC_PATH="F:/muandroid3/AndroidApp-master/app/src/main/java";
	static String xml_SRC_PATH="F:/muandroid3/AndroidApp-master/app/src/main/res/layout";
	static String manifest_SRC_PATH="F:/muandroid3/AndroidApp-master/app/src/main";
	public static Vector getNewTragetFiles(String s)
	{
		Vector targetFiles = new Vector();
		if(s=="java")
			getJavacArgForDir (java_SRC_PATH, "", targetFiles);
		else if(s=="xml")
			getXmlArgForDir(xml_SRC_PATH,"",targetFiles);
		else if(s=="manifest")
			getManifestArgForDir(manifest_SRC_PATH,"",targetFiles);
		return targetFiles;
	}

	private static String getManifestArgForDir(String dir, String str, Vector targetFiles) {
		String result = str;
		String temp = "";
		
		File dirF = new File(dir);
		File[] javaF = dirF.listFiles (new ExtensionFilter("xml"));      
		if (javaF.length > 0)
		{
			result = result + dir + "/*.xml  ";
			
			for (int k=0; k<javaF.length; k++)
			{
				temp = javaF[k].getAbsolutePath();
				temp.replace('\\', '/');
				targetFiles.add(temp.substring(manifest_SRC_PATH.length()+1, temp.length()));
			}
	      }
	...
	private static String getXmlArgForDir(String dir, String str, Vector targetFiles) {
		// TODO Auto-generated method stub
		String result = str;
		String temp = "";
		...
		if (javaF.length > 0)
		{
			...
			for (int k=0; k<javaF.length; k++)
			{
				temp = javaF[k].getAbsolutePath();
				temp.replace('\\', '/');
				targetFiles.add(temp.substring(xml_SRC_PATH.length()+1, temp.length()));
			}
	      }
	...
	protected static String getJavacArgForDir (String dir, String str, Vector targetFiles)
	{
		...
		if (javaF.length > 0)
		{
			...
			for (int k=0; k<javaF.length; k++)
			{
				temp = javaF[k].getAbsolutePath();
				temp.replace('\\', '/');
				targetFiles.add(temp.substring(java_SRC_PATH.length()+1, temp.length()));
			}
	      }
